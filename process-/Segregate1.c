...
/*
워드 rollback | 기본 작동 환경을 EC2에 한정하기로 결정
add, remove에서 listPtr parameter 추가. 기존 Explicit에 그대로 적용하기 위해 해당 수단을 동원
    #1 정확한 배치 freeList를 찾기
    #2 해당 freeList에 대해 작업하기 (삽입 또는 삭제)

아쉽게도 진짜 short1 계열만 작동하는 상황
배열 사용 금지라는 룰 때문에
find_size_class_index 로 정확하게 타겟팅하여 getListPointer에서 정확한 리스트 주소 찾아오게끔 유도

Fit 전략에 대해선 변동이 없음 : 일단 작동이 목표였기 때문에
*/
...
{ // np
    ...
    {
        int pos = find_size_class_index(asize);
        void ** targetList = getListPointer(pos);
        remove_freeList(ptr, targetList)
        place(ptr, asize);
        return ptr;
