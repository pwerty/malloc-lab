...
#define GET_FREELIST_PTR(i) (*(free_lists + i))
#define SET_FREELIST_PTR(i, ptr) (*(free_lists + i) = ptr)
...
/* 이전 블록의 포인터 가져오기 */
#define PREV_BLKP(header_p) ((char **)(header_p)-GET_TOTAL_SIZE((char **)(header_p)-FTR_SIZE))
#define NEXT_BLKP(header_p) (FTRP(header_p) + FTR_SIZE)
...
int mm_init(void)
{
    ...
    char **epilog = NEXT_BLKP(heap_ptr); // 에필로그 헤더 포인터
    PUT_WORD(epilog, PACK(0, TAKEN));             // 에필로그 헤더
    PUT_WORD(FTRP(epilog), PACK(0, TAKEN));       // 에필로그 푸터

    heap_ptr = NEXT_BLKP(heap_ptr); // heap 포인터를 프롤로그 블록 다음으로 이동시킴
...
void *mm_realloc_wrapped(void *ptr, size_t size, int buffer_size)
{
    ...
    else if (new_size > old_size)
    {
        ...
        int prev_status = GET_STATUS(PREV_BLKP(bp));
        int next_status = GET_STATUS(NEXT_BLKP(bp));
...
static void add_freeList(char **bp)
{
    ...
    char **front_ptr = GET_FREELIST_PTR(index); // 해당 가용 리스트의 주소를 받아오기
    char **prev_ptr = NULL;
    ...
    if (front_ptr == NULL)
    {
        ...
        SET_FREELIST_PTR(index, bp); // 가용 리스트의 시작 지점으로 설정
        return;
    }
    ...
    if (size >= GET_SIZE(front_ptr))
    {
        SET_FREELIST_PTR(index, bp); // 가용 리스트의 시작 지점으로 설정
        SET_PTR(GET_PTR_PRED(bp), NULL);
        SET_PTR(GET_PTR_SUCC(bp), front_ptr);
        SET_PTR(GET_PTR_PRED(front_ptr), bp);
        return;
    }
    ...
    {
        prev_ptr = front_ptr;
        front_ptr = NEXT_FREE(front_ptr);
    }

    if (front_ptr == NULL) // 가용 리스트의 끝에 도달한 경우
    {
        SET_PTR(GET_PTR_SUCC(prev_ptr), bp);
        SET_PTR(GET_PTR_PRED(bp), prev_ptr);
        SET_PTR(GET_PTR_SUCC(bp), NULL);
        return;
    }
    else
...
static void *find_free_block(size_t words)
{
    ...
    while (index <= MAX_POWER)
    {
        ...
        if ((bp = GET_FREELIST_PTR(index)) != NULL && GET_SIZE(bp) >= words)
        {
            ...
            while (1)
            {
                ...
                if (NEXT_FREE(bp) == NULL || GET_SIZE(NEXT_FREE(bp)) < words)
                    return bp;
...
static void remove_freeList(char **bp)
{
    char **prev_block = PREV_FREE(bp);
    char **next_block = NEXT_FREE(bp);
    int index;
    ...
    if (prev_block == NULL) // 해당 size class 리스트의 맨 앞에 있는 가장 큰 블록인 경우
    {
        ...
        GET_FREELIST_PTR(index) = next_block;      // 다음 블록을 가용 리스트의 첫 번째 블록으로 설정
    }
    else
    {
        SET_PTR(GET_PTR_SUCC(prev_block), next_block); // 이전 블록이 다음 블록을 가리키도록 수정
    }

    if (next_block != NULL)
    {
        SET_PTR(GET_PTR_PRED(next_block), prev_block);
    }
